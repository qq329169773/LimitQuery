...
public class MethodExecuteLogBeanHtmlLayout extends Layout {
	...
	private static String CLASS_NAME_EXECUTE = "execute";
	...
	public String format(LoggingEvent event) {
		...
		if(obj instanceof MethodExecuteLogBean){
			MethodExecuteLogBean logBean = (MethodExecuteLogBean)obj ;
			...
			sbuf.append("<td>");
			sbuf.append(MMDD_HHMMSS.format(new Date(logBean.getStartTime())));
			sbuf.append("</td>" + Layout.LINE_SEP);
			
			sbuf.append("<td>");
			sbuf.append(MMDD_HHMMSS.format(new Date(logBean.getEndTime())));
			sbuf.append("</td>" + Layout.LINE_SEP);
			...
			sbuf.append("<td>");
			sbuf.append("<font color='red'>"+logBean.getCountExecuteFailed()+"</font>"+"/"+"<span class="+CLASS_NAME_EXECUTE+">"+logBean.getCountExecute()+"</span>");
 			sbuf.append("</td>" + Layout.LINE_SEP);
			
			sbuf.append("<td>");
			sbuf.append(logBean.getMaxTimeMethodExecute()+"ms" );
			sbuf.append("</td>" + Layout.LINE_SEP);
			
			if(logBean.getCountExecute().get() > 0){
				sbuf.append("<td>");
				sbuf.append((logBean.getAvgTimeMethodExecute()  / logBean.getCountExecute().get())+"ms");
				sbuf.append("</td>" + Layout.LINE_SEP);
			}else{
				sbuf.append("<td>0</td>");
			}
			sbuf.append("<td>");
			sbuf.append(logBean.getMinTimeMethodExecute() +"ms");
	...
	public String getHeader() {
		...
		sbuf.append("<script type=\"text/javascript\">"
			   +" function showTotalExcute(){"
			   +"     var qps = $(\"."+CLASS_NAME_EXECUTE+"\");"
			   +"     var total = 0 ;"
			   +"      for (var i = qps.length - 1; i >= 0; i--) {"
			   +"        total += ($(qps[i]).html()) * 1;"
			   +"     };"
			   +"     alert(\"total execute : \" + total);"
			   +"  }     "
			   + "</script>");
		...
		sbuf.append("<button onclick=\"showTotalExcute()\">Total Excute</button>");
		...
		sbuf.append("<th>执行结果</th>" + Layout.LINE_SEP);
		sbuf.append("<th>Max Time</th>" + Layout.LINE_SEP);
		sbuf.append("<th>Avg Time</th>" + Layout.LINE_SEP);
		sbuf.append("<th>Min Time</th>" + Layout.LINE_SEP);
 		sbuf.append("</tr>" + Layout.LINE_SEP);
		return sbuf.toString();
	}
	/**
